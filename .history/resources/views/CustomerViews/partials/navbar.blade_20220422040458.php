    ...
    <div class="container">
        ...
        <div class="navbar-collapse collapse" id="navbarColor02" style="">
            <ul class="navbar-nav mr-auto d-flex align-items-center">
                <li class="nav-item">
                <a class="nav-link" href={{ route('CategoryPosts.show', '0') }}>Flash <span class="sr-only">(current)</span></a>
                </li>
                @foreach ($categories as $category)
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('CategoryPosts.show', $category->category_title) }}">{{ Str::ucfirst($category->category_title) }}</a>
                    </li>
                @endforeach
                {{-- <li class="nav-item">
