					...
					<div class="mb-3 d-flex justify-content-between">
						<div class="pr-3">
							<h2 class="mb-1 h4 font-weight-bold">
								<a class="text-dark" href="{{ route('article.show', compact('post')) }}">
									{{ $post->post_title }}
								</a>
							</h2>
							<p>
								{{ Str::limit($post->post_content, 75, ' ...') }}
								@if ( Str::length($post->post_content) > 75)
									<a href="{{ route('article.show', $post->post_title) }}">Voir Plus</a>	
								@endif
							</p>
							<div class="card-text text-muted small">
								<span>Source : </span> {{ $post->post_source }}
							</div>
							<small class="text-muted">
								{{ $post->created_at->format('d M Y') }}
								&middot; Rubrique &middot; {{ Str::ucfirst($post->category->category_title) }}
							</small>
						</div>
						<img height="120" src="{{ $post->post_img_path }}">
					</div>	
				@endforeach
			</div>
